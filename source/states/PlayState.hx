...
class PlayState extends MusicBeatState
{
	...
	*/
	...
	inline private function createCountdownSprite(image:String, antialias:Bool):FlxSprite
	{
		...
		{
			spr.setGraphicSize(Std.int(spr.width * 0.1));
			FlxTween.tween(spr.scale, {x:1, y:1}, Conductor.crochet / 1000, {ease: FlxEase.elasticOut});
			delay = 0.35;
		}
		...
		FlxTween.tween(spr, {/*y: spr.y + 100,*/ alpha: 0}, Conductor.crochet / 1000, {
			ease: FlxEase.cubeInOut, startDelay: delay,
	...
	function globalNoteHit(note:Note, isPlayer:Bool):Void
	{
		...
		var char:Character = isPlayer ?  boyfriend : dad;
		if(note.gfNote || SONG.notes[curSection] != null && SONG.notes[curSection].gfSection) char = gf;
	...
	override function sectionHit()
	{
		if (SONG.notes[curSection] != null)
		{
			if (generatedMusic && !endingSong && !isCameraOnForcedPos)
				...
				if (camZooming && FlxG.camera.zoom < 1.35  && !isZooming  && ClientPrefs.data.camZooms)
				{
